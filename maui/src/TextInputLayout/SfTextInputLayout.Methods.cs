...
namespace Syncfusion.Maui.Toolkit.TextInputLayout
{
	public partial class SfTextInputLayout
	{
		...
		/// <summary>
		/// Calculates the interpolated value between two points (start and end) based on the progress ratio.
		/// </summary>
		/// <param name="start">The starting value of the interpolation.</param>
		/// <param name="end">The ending value of the interpolation.</param>
		/// <param name="progress">A value between 0 and 1 representing the interpolation progress.</param>
		/// <returns>The interpolated value as a float.</returns>
		float Interpolate(double start, double end, double progress)
		{
			return (float)(start + (end - start) * progress);
		}

		/// <summary>
		/// Updates the translation animation by interpolating the X and Y positions 
		/// based on the given progress value.
		/// </summary>
		/// <param name="progress">
		/// A value between 0 and 1 representing the current progress of the animation.
		/// </param>
		void UpdateTranslateAnimation(double progress)
		{
			_isAnimating = true;

			// Interpolate X and Y values based on the progress
			if (IsHorizontalAnimationNeeded())
			{
				_hintRect.X = Interpolate(!IsHintFloated ? _translateEndX : _translateStartX,
										  IsHintFloated ? _translateEndX : _translateStartX, progress);
			}
			_hintRect.Y = Interpolate(_translateStartY, _translateEndY, progress);
		...
		internal void StartAnimation()
		{
			...
			if (!IsHintFloated || IsHorizontalAnimationNeeded())
			{
				UpdateHintPosition();
			}
			...
			// Create the animation for translating both X and Y
			var translateAnimation = new Animation(UpdateTranslateAnimation, 0, 1, Easing.SinIn);

			// Add scaling animation concurrently
			translateAnimation.WithConcurrent(scalingAnimation, 0, 1);

			// Commit the animation
			translateAnimation.Commit(this, "showAnimator",
				rate: 7,
				length: (uint)DefaultAnimationDuration,
				finished: OnTranslateAnimationEnded,
				repeat: () => false);
		...
		/// <summary>
		/// Determines whether a horizontal animation is required based on the alignment and styling conditions.
		/// </summary>
		/// <returns>
		/// Returns <c>true</c> if horizontal animation is required; otherwise, <c>false</c>.
		/// </returns>
		bool IsHorizontalAnimationNeeded()
		{
			return (ShowUpDownButton && ((IsUpDownVerticalAlignment && IsUpDownAlignmentLeft) || (!IsUpDownVerticalAlignment && (IsUpDownAlignmentLeft || IsUpDownAlignmentBoth))) && (IsNone || IsOutlined));
		}
		...
		void UpdateHintPosition()
		{
			...
			if (ShowLeadingView && LeadingView != null)
			{
				...
				if (!IsFilled && IsHintFloated && ShowUpDownButton)
				{
					_translateEndX = _hintRect.X;
				}
			}

			if ((!IsHintFloated || IsFilled) && ShowUpDownButton && (IsUpDownAlignmentLeft || IsUpDownAlignmentBoth) && ((IsUpDownVerticalAlignment && !IsUpDownAlignmentBoth) || !IsUpDownVerticalAlignment))
			{
				...
				if (IsNone)
				{
					_hintRect.X += (float)((BaseLineMaxHeight * 3) + DefaultAssistiveLabelPadding);
				}
				else
				{
					_hintRect.X -= (float)((BaseLineMaxHeight * 3) + DefaultAssistiveLabelPadding);
			...
			if (!IsFilled && ShowUpDownButton && !IsHintFloated)
			{
				_translateStartX = _hintRect.X;
			}
		}
		...
		void UpdateStartAndEndValue()
		{
			if (IsNone && (Width > 0 && Height > 0))
			{
				if (IsHintFloated)
				{
					_translateStartY = DefaultAssistiveLabelPadding;
					_translateEndY = Height - HintSize.Height - BaseLineMaxHeight - (DefaultAssistiveLabelPadding * 2) - AssistiveLabelPadding - TotalAssistiveTextHeight() - DefaultAssistiveLabelPadding;
				}
				else
				{
					_translateStartY = Height - HintSize.Height - BaseLineMaxHeight - (DefaultAssistiveLabelPadding * 2) - AssistiveLabelPadding - TotalAssistiveTextHeight() - DefaultAssistiveLabelPadding;
					_translateEndY = DefaultAssistiveLabelPadding;
				}
			}
			...
			if (IsOutlined && (Width > 0 && Height > 0))
			{
				...
				{
					_translateStartY = 0;
					_translateEndY = _outlineRectF.Center.Y - (HintSize.Height / 2);
				}
				else
				{
					_translateStartY = _outlineRectF.Center.Y - (HintSize.Height / 2);
					_translateEndY = 0;
				}
			}
			...
			if (IsFilled && (Width > 0 && Height > 0))
			{
				if (IsHintFloated)
				{
					_translateStartY = (float)((DefaultAssistiveLabelPadding * 2) + (DefaultAssistiveLabelPadding / 2));
					_translateEndY = ((Height - TotalAssistiveTextHeight() - AssistiveLabelPadding) / 2) - (HintSize.Height / 2);
				}
				else
				{
					_translateStartY = ((Height - TotalAssistiveTextHeight() - AssistiveLabelPadding) / 2) - (HintSize.Height / 2);
					_translateEndY = (float)((DefaultAssistiveLabelPadding * 2) + (DefaultAssistiveLabelPadding / 2));
				}
			}
		}
